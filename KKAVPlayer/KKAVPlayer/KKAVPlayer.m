...
-(void)addProgressObserver{
    ...
    self.playTimeObserverObject = [self.player addPeriodicTimeObserverForInterval:CMTimeMake(1.0, 1.0) queue:dispatch_get_main_queue() usingBlock:^(CMTime time) {
        ...
        strongSelf.totalTime = CMTimeGetSeconds([playerItem duration]);
        strongSelf.curtPosition = strongSelf.currentPlayTime/strongSelf.totalTime ;
        
        if(strongSelf.progressCallback){
            strongSelf.progressCallback(strongSelf,strongSelf.curtPosition);
        }
        if(strongSelf.delegate && [strongSelf.delegate respondsToSelector:@selector(player:progress:)]){
            [strongSelf.delegate player:strongSelf progress:strongSelf.curtPosition];
        }
    }];
}
...
- (void)setSeekToPosition:(float)seekToPosition{
    ...
    [self.player seekToTime:time completionHandler:^(BOOL finish){
        __strong typeof(weakSelf)strongSelf = weakSelf;
        if(finish){
            strongSelf.currentPlayTime = time.value;
            if(strongSelf.seekCompleteCallback){
                strongSelf.seekCompleteCallback(strongSelf, strongSelf.curtPosition, seekToPosition);
            }
            if(strongSelf.delegate && [strongSelf.delegate respondsToSelector:@selector(playerSeekComplete:prePos:curtPos:)]){
                [strongSelf.delegate playerSeekComplete:strongSelf prePos:strongSelf.curtPosition curtPos:seekToPosition];
            }
            if(isPlay){
                [strongSelf play];
            }
